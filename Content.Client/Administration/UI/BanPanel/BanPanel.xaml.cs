...
public sealed partial class BanPanel : DefaultWindow
{
    ...
    private static bool _isMessageRegistered = false;
    ...
    public BanPanel()
    {
        ...
        if (!_isMessageRegistered)
        {
            _netManager.RegisterNetMessage<MsgReceiveGhostRoleNames>(OnReceiveGhostRoleNames);
            _isMessageRegistered = true;
        }
        ...
        //TODO lets come back to this after antags.
        RequestGhostRoleNames();
        // CreateRoleGroup("GhostRoles", prototypeManager.EnumeratePrototypes<EntityPrototype>()
        //     .Where(filteredPrototype => filteredPrototype.HasComponent<GhostRoleComponent>())
        //     .Select(ghostComp => ghostComp.Name), Color.Cyan);
    }


    private void RequestGhostRoleNames()
    {
        var session = _netManager.ServerChannel;
        if (session != null)
        {
            var request = new MsgRequestGhostRoleNames
            {
                UserId = session.UserId // Ensure UserId is set properly
            };
            _netManager.ClientSendMessage(request);
        }
    }

    private void OnReceiveGhostRoleNames(MsgReceiveGhostRoleNames msg)
    {
        if (_isPanelOpen)
        {
            CreateRoleGroup("GhostRoles", msg.RoleNames, Color.Cyan);
        }
        else
        {
            _banpanelSawmill.Warning("Received ghost role names but the panel is already closed.");
        }
    }
